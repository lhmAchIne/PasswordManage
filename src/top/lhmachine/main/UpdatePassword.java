...
public class UpdatePassword extends JFrame implements ActionListener{
	...
	private JButton jb_select, jb_back, jb_update, jb_finish = null;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand() == "返回目录") {
			...
			if (jtf1.getText().isEmpty() || jtf2.getText().isEmpty()) {
				JOptionPane.showMessageDialog(null, "网站名和用户名不可以为空，请先查询!", "提示消息", JOptionPane.WARNING_MESSAGE);
			}else {
				if (jtf3.getText().isEmpty()) {
					JOptionPane.showMessageDialog(null, "密码不可以为空!", "提示消息", JOptionPane.WARNING_MESSAGE);
				}else {
					if (dbHelper.updateData(jtf1.getText().toString(), jtf2.getText().toString(), jtf3.getText().toString())) {
